...
const IdeaDetailDialog = ({idea, onClose}) => {
    ...
    const fetchComments = async () => {
        ...
        try {
            const response = await fetch(`http://127.0.0.1:8000/api/ideas/${idea.id}/comments`, {
                headers: {
            ...
            <div className="bg-white rounded-lg overflow-y-auto max-h-full w-full max-w-3xl p-6 relative">
                ...
                <div className="mb-6 border-b pb-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <span className="text-gray-600">{idea.likes}</span>
                    </div>
                    <div>
                        <span className="font-semibold text-gray-700">Dislikes: </span>
                        <span className="text-gray-600">{idea.dislikes}</span>
                    </div>
                </div>
